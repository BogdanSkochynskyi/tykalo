...
**Rules:**
- A list always has exactly one OWNER
- OWNER can transfer ownership; can't be removed by anyone — must transfer first. Transfer (`ListMemberService.transferOwnership`, TK-194) demotes the old owner to EDITOR (creating that row if needed), promotes the target to OWNER **and reassigns `lists.owner_id`** (the column is now mutable, so `owner_id` stays the authority source)
...
- TK-197 backfills `list_members` rows for existing lists (owner → OWNER role) via a one-shot Flyway data migration (`V18__backfill_list_members.sql`): `INSERT … SELECT … WHERE NOT EXISTS`, idempotent (re-running on a later deploy changes nothing), `status` omitted so it defaults to `ACTIVE` (V16), `invited_by` NULL. Auto-runs on next deploy; `lists.owner_id` stays the authority source (cleanup deferred to Phase 2+)
- **Invites are PENDING memberships (TK-193).** `list_members` carries a `status` (`PENDING`/`ACTIVE`, V16) and `invited_by`; an invite creates a `PENDING` row that grants *nothing* until accepted. `ListPermissionService.resolveRole` and `ListService.findAllAccessibleLists` count **ACTIVE only**, so a pending invitee has no access/visibility. Accept flips PENDING→ACTIVE (`ListInviteService.respond`); decline deletes the row. Two entry points (`ListInviteService`): by `@username` (registered → PENDING row + Yes/No prompt; else offer a share-link) and a `t.me/<bot>?start=list_invite_<base64({listId,invitedBy,role})>` deep-link whose validity is gated by a 7-day Redis token (`list_invite:{payload}`) — handled in `StartCommandHandler` like the nudger one. The Yes/No prompt (`ListInvitePromptService`) callbacks are `lm:accept|decline:{memberId}` (`ListInviteCallbackHandler`), which notify the inviter.
- **Members screen (TK-194).** `👥 Members` from the list view (`lv:members:{list}`) opens `menu/MembersService` (state `MembersScreen(listId)`, `ms:` callback prefix, `menu/handler/MembersCallbackHandler`): members listed OWNER→EDITOR→MEMBER, with a `🗑 Remove` button per non-OWNER (OWNER/EDITOR only), `➕ Invite by username` (reuses the `InviteMemberService` typed sub-flow, which now cancels back here via `ms:invx`), `🔗 Get share link`, and — for the OWNER — `👑 Transfer ownership`. A MEMBER sees the list read-only. The owner is **synthesized from `lists.owner_id`** when no OWNER row exists (menu-created lists have no `list_members` row until someone is invited). Remove/transfer mutations live in `list/ListMemberService` (predicate-gated, returning `RemoveMemberResult`/`TransferOwnershipResult`); the handler notifies the removed user / new owner and re-renders the screen.
...
**Notification aggregator (TK-196, `io.tykalo.notification`).** Attributable user actions publish a `ListActivityEvent(listId, actorId, kind, count)` — fired only from the actor-aware `TaskService` overloads (`createTask`/`createTasks`/`markDone` with an `actorId`; the menu handlers now call those), so recurrence/system paths sync live but never push. `ListChangeAggregator` listens AFTER_COMMIT, resolves recipients (the list's ACTIVE members ∪ the synthesized owner when no OWNER row exists, minus the actor — so a private list has no recipients) and routes each by their preference into a `NotificationBuffer` (interface; `RedisNotificationBuffer` in prod, an in-memory double in tests). INSTANT/BATCHED are windowed buckets (Redis hash per (recipient, list, mode), indexed by a flush-time ZSET; window start fixed via `ZADD NX`); DAILY_DIGEST is a per-recipient hash. `ListChangeNotificationJob` flushes due windows every 20s and daily digests hourly (both ShedLock-guarded); a flush landing in quiet hours is rescheduled to `QuietHoursService.nextActiveAt` rather than dropped. `NotificationMessageFormatter` builds the MarkdownV2 text.
...
- **Rich messages (keyboards, edit-in-place)** — the `String`/`Optional<String>` reply path only sends plain text and discards the sent message's id. When a handler needs an inline keyboard, the returned `message_id`, or an in-place edit, it goes through `TelegramMessageGateway` (`sendMarkdown`/`editMarkdown`, MarkdownV2) instead of returning a reply. The real impl is wired only when `telegram.bot.polling.enabled` is true; non-polling contexts (tests) get a `NoOpTelegramMessageGateway` fallback so dependent beans still construct. First use (TK-124): `ListMessageService` publishes a list's "live" editable message — one self-updating message per (list, chat), its `tg_message_id` stored in `list_messages` — and `ListRenderer` produces the MarkdownV2 body + per-task inline keyboard (a TODO task gets `✅` → `task:done:{taskId}`, a DONE task gets `↩️` → `task:undo:{taskId}`). **`editMarkdown` returns an `EditOutcome`** (`EDITED`/`MESSAGE_GONE`/`FAILED`, TK-195): the gateway classifies a Telegram edit failure — a whitelisted 400 (`message to edit not found`, `can't be edited`, too old) or a 403 (chat unreachable) → `MESSAGE_GONE`; everything else, incl. `message is not modified`, parse errors and 429, → `FAILED` so a transient/formatting fault never drops a valid record. Only `ListMessageService` consumes it (other callers ignore the return): on `MESSAGE_GONE` it deletes that `list_messages` row so the bot stops chasing a dead message. This is what makes a shared list update for every member at once — `onListChanged` (AFTER_COMMIT on `ListChangedEvent`) re-renders **every** row `findByListId` returns, one edit per chat, pruning gone rows as it goes (TK-195).
...
- **First-`/start` onboarding (TK-172)** — a genuine first contact with no Nudger-invite deep-link kicks off a Redis-backed, skip-able 3-step flow owned by `onboarding/OnboardingService` (greeting + Nudgers concept → create a first Shopping list → how to add Nudgers). `UserService.register(Update)` returns `Registration(user, created)` so `StartCommandHandler` runs onboarding only on `created` (and stays silent — onboarding owns the messages via `TelegramMessageGateway`); invited users are unaffected; returning users now land on the main menu (TK-181). One message evolves in place; state is the current `OnboardingStep` stored at `user:{id}:onboarding_step` (7-day TTL, same `StringRedisTemplate` pattern as `CurrentContextService`, no DB). Buttons land on `OnboardingCallbackHandler` under the `onb:` callback prefix (`onb:go`/`onb:list`/`onb:invite`/`onb:skip`); every transition is **guarded by the current step**, so a replayed/stale button after completion is a harmless no-op (e.g. "Create it" can't make a second list). Callback prefixes in use so far: `task:` (live-message toggles), `nudger:` (consent), `tn:` (per-task nudger picker), `onb:` (onboarding), `menu:` (main menu), `lists:` (My Lists screen), `lv:` (list view, incl. `lv:members` → Members screen, TK-194), `lm:` (shared-list invite accept/decline, TK-193), `ms:` (Members screen: remove/transfer/invite/share, TK-194), `add:` (add-items prompt Done/Cancel), `create:` (new-list type picker / cancel), `help:` (help screen category drilldown / back), `set:` (settings screen: `set:notif:{PREF}` notification-preference radio / `set:menu` back, TK-196).
- **Menu UX (TK-181+, Phase 1.5a)** — menu-driven navigation built on the Redis conversation state (TK-187). `menu/MenuService.showMainMenu(User)` sends the inline-keyboard entry screen via `TelegramMessageGateway` and sets `ConversationState → MainMenu`; the `/menu` command (`MenuCommandHandler`) and a returning user's `/start` both open it. Buttons carry a `menu:*` `callback_data` routed to `MenuCallbackHandler`. Screens navigate by **editing the same message in place** (the callback's `message_id`), so each screen service offers a send-new entry (for its command) and an edit-in-place entry (for navigation). Built so far: `menu:my_lists` → the **My Lists screen** (TK-182, `menu/MyListsService`) — active lists as one tappable button each (`lists:open:{id}`, 2 per row, page size 8 with `lists:page:{n}`), a bottom row `➕ New list` (`lists:new`) / `⬅️ Back to menu` (`lists:menu`), and an Inbox-only empty state. It replaces the old `/lists` text listing (the command now lives in `menu/MyListsCommandHandler`; `lists:` actions are routed by `MyListsCallbackHandler`). `lists:open:{id}` → the **list view** (TK-183, `menu/ListViewService`) — the primary working screen for one list: header `{icon} {name}`, items with a ✅/☐ indicator (20/page, `lv:page:{list}:{n}`), one toggle button per item (`lv:done|undo:{task}:{page}` — the page rides the `callback_data` so the view re-renders the same page in place), plus Action (`➕ Add items` → `lv:add`), Settings (`👥 Members` → `lv:members`, the Members screen TK-194; `⋯ More` → `lv:more`) and Nav (`⬅️ Back to lists` → `lv:lists`) rows; routed by `ListViewCallbackHandler`. Toggling still calls `TaskService.markDone`/`reopen` (which fire `ListChangedEvent`), so the legacy TK-B001 auto live message stays in sync — the list view is the new primary surface, not a second source of truth. `lv:add:{list}` → the **add-items flow** (TK-184, `menu/AddItemsService`) — the friendly replacement for multi-line bulk-add: it sets `ConversationState → AddingItems(listId, listViewMessageId)` and sends a separate prompt message below the list view with `✅ Done` (`add:done`) / `❌ Cancel` (`add:cancel`). While in that state, each plain-text message is routed (by the dispatcher's `StateHandler` path) to `AddItemsStateHandler` → one task per non-blank line, then the list-view message above is re-rendered in place at its last page so the new item shows immediately; the flow stays silent (the live view is the feedback). Done/Cancel (`AddItemsCallbackHandler`) delete the prompt and return to the list view — both keep items already added; a `/command` in the state exits it via the dispatcher, and a stale prompt button after the flow ended just removes the prompt. The legacy multi-line `BulkAddHandler` still works as a shortcut outside the state. `menu:create` / `lists:new` → the **new-list flow** (TK-185, `menu/CreateListService`) — the conversational replacement for `/list create`: it edits the opening message in place into a **type picker** (`🛒 Checklist` / `📋 Project` / `🔄 Routine` + `❌ Cancel`, `create:type:{TYPE}` / `create:cancel`) and sets `ConversationState → CreatingListType`; picking a type edits it into a `✏️ Name your list:` prompt and sets `CreatingListName(type, promptMessageId)` (carrying that message id). The next plain-text message is routed (via the `StateHandler` path) to `CreateListStateHandler` → `CreateListService.submitName`, which validates (non-blank, no duplicate active name per user — re-prompts the same message in place on failure, staying in the state) and on success creates the list and transitions the prompt message into its list view (TK-183). `❌ Cancel` at either step returns to the main menu. `create:type:*` is guarded by the current state (a stale picker after the flow moved on is a no-op) and rejects `INBOX`; the ROUTINE "set up schedule now?" follow-up is deferred to TK-203. Routed by `CreateListCallbackHandler`. Still placeholder toasts: More → TK-186, Stats. (`👥 Members` opens the Members screen, TK-194 — see the Permission model section.) `menu:settings` / `/settings` → the **settings screen** (TK-196, `menu/SettingsService`, state `ConversationState → Settings`, `set:` callback prefix, `SettingsCommandHandler`/`SettingsCallbackHandler`) — a "List change notifications" section with one radio button per `ListChangeNotificationPreference` (the current one marked, others switchable); picking one persists via `UserService.setListChangeNotifications` and re-renders in place, `⬅️ Back to menu` returns to the main menu. `MainMenu`/`Lists`/`ListView` are not input-expecting, so text commands keep working alongside the menu. `menu:help` / `/help` → the **help screen** (TK-189, `menu/HelpService`) — replaces the old static text `/help` (TK-171) with inline navigation: a short intro plus one button per `menu/HelpTopic` (`📋 Lists & tasks` / `🔔 Nudgers` / `⏰ Scheduling & timezone` / `⚙️ Settings`, `help:cat:{TOPIC}`) and `⬅️ Back to menu` (`help:menu`), state `ConversationState → Help`; tapping a category edits in place to that topic's command catalogue as plain markdown (text-only like TK-171) with a `⬅️ Back` (`help:back`) to the top level, state `HelpCategory(topic)`. The command (`HelpCommandHandler`, still under `user/handler`) sends a new message; the menu button and Back edit in place; routed by `HelpCallbackHandler`. Both help states are navigation (no text input).
